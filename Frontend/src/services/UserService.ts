...
export default class UserService {
  ...
  public static registerUser = async (): Promise<string> => {
    try {
      ...
      console.log("registeruser", this.serverBaseURL);
  ...
  public static addVideoIdToUser = async (
    user_id: string,
    video_id: string
  ): Promise<boolean> => {
    try {
  ...
  public static addEmotionsToUser = async (
    user_id: string,
    emotions: {
      expression: string;
      probability: number;
  ...
  public static disconnectUser = async (user_id: string): Promise<boolean> => {
    try {
      let response = await fetch(
        `${this.serverBaseURL}/${user_id}/disconnect`,
        {
          method: "post",
          headers: {
            "Content-Type": "application/json",
